...
- Name: metrics.spec.ts >> Metrics & Components Functionality >> Metric Cards display varying risk colors and trends
- Location: tests\e2e\metrics.spec.ts:67:7
...
Locator: locator('div').filter({ hasText: 'VIX (Volatility)' }).nth(1)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('div').filter({ hasText: 'VIX (Volatility)' }).nth(1)
